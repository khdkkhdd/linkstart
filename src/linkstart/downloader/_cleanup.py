...
from linkstart.downloader._base import _parse_epoch, unique_path
...
def _new_coverage_seconds(candidate: Interval, covered: list[Interval]) -> int:
    """How many seconds of `candidate` are NOT already in any `covered` interval.

    Intervals are half-open [start, end). Covered intervals may overlap each
    other; this implementation clips them to the candidate range, merges, then
    subtracts — so over-counting doesn't happen when two kept edges overlap.
    """
...
class _DualCleanupMixin:
    def _edge_keep_path(self, base_final: Path, index: int) -> Path:
        return base_final.with_name(f"{base_final.stem}.edge_{index:03d}.mp4")

    def _recovered_keep_path(self, base_final: Path, index: int) -> Path:
        return base_final.with_name(f"{base_final.stem}.recovered_{index:03d}.mp4")

    async def _cleanup_dual(
        self,
        ...
        *,
        ...
        # `full.*.mp4` also matches yt-dlp's intermediate per-format files
        # like `full.{epoch}.f140.mp4`. Filter by _parse_epoch so only the
        # final-remuxed `full.{epoch}[_NN].mp4` outputs are base candidates;
        # intermediates are handled by _recover_fragments later.
        full_files = [
        ...
        for f in full_files:
            d = await self._ffprobe_duration(f)
            if d is not None:
                full_durations[f] = d

        if not full_durations:
            return await self._cleanup_no_base(
                channel, live, parts_dir, retry_count=retry_count
            )
        ...
        base_final = unique_path(self._final_path(channel, live))
        ...
        for f, epoch, d in edge_entries:
            if d is None:
                ...
                kept = self._edge_keep_path(base_final, keep_index)
                f.rename(kept)
                extras.append(kept)
                keep_index += 1
                continue
            ...
            if trust_base and new_cov < self.MIN_UNIQUE_COVERAGE_SEC:
                f.unlink(missing_ok=True)
            else:
                kept = self._edge_keep_path(base_final, keep_index)
                f.rename(kept)
        ...
        extras += await self._recover_fragments(parts_dir, base_final, covered)
    ...
    async def _cleanup_no_base(
        ...
        *,
        ...
        """Fallback when no usable full.*.mp4 was produced.

        First try `edge.*.mp4`. If none, attempt to recover yt-dlp's
        intermediate per-format fragment pairs (`*.f<itag>.mp4(.part)?`),
        promoting the first remuxed result to base.
        """
        edge_files = [
        ...
        if edge_files:
            ...
            for i, f in enumerate(edge_files[1:], start=1):
                kept = self._edge_keep_path(base_final, i)
                f.rename(kept)
                extras.append(kept)
        else:
            # Last-ditch: try fragment recovery into a fresh base.
            base_final = unique_path(self._final_path(channel, live))
            recovered = await self._recover_fragments(parts_dir, base_final, covered=[])
            if not recovered:
                return DownloadResult(
                    success=False,
        ...
        base_duration = await self._ffprobe_duration(base_final) or 0
        shutil.rmtree(parts_dir, ignore_errors=True)
    ...
    async def _recover_fragments(
        ...
        """Find video/audio fragment pairs (*.f<itag>.mp4 or .mp4.part), remux
        those that add unique coverage beyond `covered`. yt-dlp may have
        finalized one side (audio.f140.mp4) while the other is still .part —
        we accept either suffix and pair by the prefix-before-".f"."""
        ...
        for prefix, members in groups.items():
            ...
            d = await self._ffprobe_duration(video)
            if d is not None:
                # Approximate the recording epoch by the file's mtime.
                epoch = int(video.stat().st_mtime)
                frag_iv: Interval = (epoch, epoch + d)
                if _new_coverage_seconds(frag_iv, covered) < self.MIN_UNIQUE_COVERAGE_SEC:
                    continue   # already covered → skip

            target = self._recovered_keep_path(base_final, keep_index)
            if await self._ffmpeg_remux(video, audio, target):
    ...
    async def _ffprobe_duration(self, path: Path) -> int | None:
        """Return integer seconds of `path` via ffprobe, or None on failure."""
        try:
            returncode, stdout, _ = await self._run_proc(
                [
                    self.ffprobe_bin,
                    "-v", "error",
    ...
    async def _ffmpeg_remux(self, video: Path, audio: Path, target: Path) -> bool:
        """Remux a video+audio part file pair into a playable mp4."""
        try:
            returncode, _, stderr = await self._run_proc([
    ...
    async def _remux_ts_to_mp4(self, part: Path, final: Path) -> bool:
        """Rewrite an HLS .ts part into a standard .mp4 container without
        re-encoding. Each edge-only attempt produces its own part file, and
        each is remuxed independently so inter-attempt gaps remain visible
        instead of being silently swallowed by ffmpeg concat.
        """
        returncode, _, stderr = await self._run_proc([
            self.ffmpeg_bin, "-y", "-i", str(part),
            "-c", "copy", str(final),
        ])
        if returncode != 0:
